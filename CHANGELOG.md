...
### Fixed
- BUG-037: concluir cliques no DOM deixa de significar aposta executada. Antes do primeiro alvo financeiro o executor exige saldo legível; depois dos cliques, somente o débito observado exatamente pela exposição esperada, dentro da tolerância configurada, produz `EXECUTADA`.
- Callback `EXECUTADA` sem evidência `SALDO_DEBITADO`, ou com valores inconsistentes, é rebaixado pelo Node para `AMBIGUA`. A intenção não vira `PENDENTE`, não incrementa `entradas_feitas` e o Auto-Trader é bloqueado para impedir novas exposições até revisão humana.
- A auditoria persiste método, saldo anterior/posterior, débito e horário da confirmação. Registros históricos sem esses campos permanecem rastreáveis como legados não comprovados e deixam de compor volume/P&L comprovado no PDF.
- O relatório de auditoria passa para A4 paisagem, largura fixa e quebra de linhas/páginas segura. Rótulos enganosos como “lucro real”, “turnover executado” e “hash de autenticidade” são substituídos por métricas qualificadas e um ID local sem pretensão criptográfica.
...
- `executar_aposta_na_tela` retorna estado estruturado: falha antes de qualquer clique de alvo é definitiva, falha após clique parcial é ambígua e, desde o BUG-037, sucesso exige evidência financeira externa de aceite pelo débito do saldo disponível.
