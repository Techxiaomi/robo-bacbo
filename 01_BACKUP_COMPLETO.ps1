...
function Find-MariaDbBin {
    ...
    foreach ($dir in ($candidates | Sort-Object Name -Descending)) {
        ...
        if ((Test-Path (Join-Path $bin 'mariadb.exe')) -and (Test-Path (Join-Path $bin 'mariadb-dump.exe'))) {
            return $bin
        }
    }
    ...
    if ($client -and $dump) {
        return Split-Path -Parent $client.Source
    }
...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    $OutputDirectory = Join-Path $ProjectRoot 'backups\full'
}
...
if ([string]$envMap['DB_HOST'] -notin @('127.0.0.1','localhost','::1')) {
    throw "Backup completo foi projetado para o MariaDB local. DB_HOST atual: $($envMap['DB_HOST'])"
}
...
try {
    ...
    if (-not $IncludeDependencies) {
        $excludedDirs += @('node_modules','venv','.venv')
    }
    ...
    $manifest = [ordered]@{
        ...
        database = [ordered]@{
            ...
            dump_file = 'database/bacbo.sql'
            dump_size_bytes = $dumpInfo.Length
            dump_sha256 = $dumpHash
        }
        project = [ordered]@{
            git_commit = $gitCommit
            git_branch = $gitBranch
            dependencies_included = [bool]$IncludeDependencies
        }
        runtime = [ordered]@{
        ...
        restore = [ordered]@{
            script = '02_RESTAURAR_SISTEMA.ps1'
            default_target = 'D:\Projetos\Bacbo'
        }
    }
