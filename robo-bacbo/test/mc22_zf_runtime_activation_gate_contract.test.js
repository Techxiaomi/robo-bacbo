...
test('MC22-Z-F: BACBO_INT continua dispensando opt-in de runtime', () => {
    const int = mesaContext.resolverMesaConhecida('BACBO_INT');
    assert.equal(
        mesaContext.afirmarMesaRuntimeHabilitada(int, {}).codigo,
        'BACBO_INT'
    );
});
...
test('MC22-Z-F: BACBO_BR sem opt-in continua fail-closed', () => {
    ...
    assert.throws(
        () => mesaContext.afirmarMesaRuntimeHabilitada(br, {}),
...
test('MC22-Z-F: opt-in BR sem round TipMiner falha fechado', () => {
    ...
    assert.throws(
        ...
        erro => erro && erro.code === 'MESA_RUNTIME_FONTE_AUSENTE'
    );
});
...
test('MC22-Z-F: round BR precisa ter formato UUID', () => {
    ...
    assert.throws(
        () => mesaContext.afirmarMesaRuntimeHabilitada(br, {
            BACBO_MESA_RUNTIME_ENABLED: '1',
            TIPMINER_BACBO_ROUND_ID: 'round-invalido'
        }),
        erro => erro && erro.code === 'MESA_RUNTIME_FONTE_INVALIDA'
    );
});
...
test('MC22-Z-F: BR nao pode reutilizar round conhecido da INT', () => {
    ...
    assert.throws(
        () => mesaContext.afirmarMesaRuntimeHabilitada(br, {
            BACBO_MESA_RUNTIME_ENABLED: '1',
            TIPMINER_BACBO_ROUND_ID: mesaContext.TIPMINER_ROUND_ID_INT_CONHECIDO
        }),
        erro => erro && erro.code === 'MESA_RUNTIME_FONTE_CRUZADA'
    );
});

test('MC22-Z-F: BR com opt-in e round proprio passa pelo gate de runtime', () => {
    comEnv(
        {
            BACBO_MESA_CODIGO: 'BACBO_BR',
            BACBO_MESA_RUNTIME_ENABLED: '1',
            TIPMINER_BACBO_ROUND_ID: ROUND_VALIDO_BR
        },
        () => {
            const mesa = mesaContext.mesaConfigurada();
            assert.equal(mesa.codigo, 'BACBO_BR');
            assert.equal(mesa.runtime_habilitado, false);
            assert.equal(mesa.runtime_ativacao_explicita, true);
        }
    );
});
...
test('MC22-Z-F: financeiro autorizado para INT e BR; mesa desconhecida permanece bloqueada', () => {
    ...
    assert.equal(financeiro.mesaFinanceiraPermitida('BACBO_X'), false);
});
