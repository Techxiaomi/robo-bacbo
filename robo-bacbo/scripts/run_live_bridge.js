...
async function findRuntimeConfig(service, tableKey, safety) {
    ...
    if (!house) throw new Error(`LIVE_BRIDGE_HOUSE_NOT_FOUND: ${HOUSE_ADAPTER_KEY}`);
...
async function main() {
    ...
    try {
        ...
        const config = await findRuntimeConfig(service, tableKey, safety);
