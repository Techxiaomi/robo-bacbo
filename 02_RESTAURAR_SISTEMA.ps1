[CmdletBinding()]
param(
    [Parameter(Mandatory=$true, Position=0)][string]$ArquivoBackup,
    [string]$TargetRoot = '',
    [switch]$Force,
    [switch]$TestMode
)
...
function Read-EnvFile([string]$Path) {
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) { throw "Arquivo .env nao encontrado no backup: $Path" }
    $map = @{}
    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($InputFile)) {
            ...
            try {
                ...
                if ($code -ne 0) { $details = @($output) + @($stderr); throw "MariaDB retornou exit code $code.`n$($details -join [Environment]::NewLine)" }
            }
            finally {
                $ErrorActionPreference = $oldEap
            ...
            $stderrFile = Join-Path $env:TEMP ('bacbo_mariadb_stderr_' + [guid]::NewGuid().ToString('N') + '.txt')
            try {
                $process = Start-Process -FilePath $Exe -ArgumentList $Arguments -NoNewWindow -Wait -PassThru -RedirectStandardInput $InputFile -RedirectStandardOutput $stdoutFile -RedirectStandardError $stderrFile
...
function Escape-SqlLiteral([string]$Value) { return $Value.Replace('\','\\').Replace("'","''") }

$backupPath = (Resolve-Path -LiteralPath $ArquivoBackup).Path
if ([IO.Path]::GetExtension($backupPath).ToLowerInvariant() -ne '.zip') { throw 'O arquivo de backup deve ser um .zip gerado por 01_BACKUP_COMPLETO.ps1.' }
...
try {
    ...
    foreach ($requiredPath in @($manifestPath,$hashListPath,$dumpFile,$projectSource,$envSource)) {
        if (-not (Test-Path -LiteralPath $requiredPath)) { throw "Backup invalido ou incompleto. Ausente: $requiredPath" }
    }

    $manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
    if ([int]$manifest.schema_version -ne 1) { throw "Versao de manifesto nao suportada: $($manifest.schema_version)" }
    ...
    if ($actualDumpHash -ne [string]$manifest.database.dump_sha256) { throw 'SHA256 do dump diverge do manifesto. Restore abortado.' }

    # Windows PowerShell 5.1 pode entregar o array JSON como um unico objeto de pipeline.
    # O foreach da linguagem enumera os itens corretamente; achatamos explicitamente.
    $parsedHashJson = Get-Content -LiteralPath $hashListPath -Raw | ConvertFrom-Json
    $hashEntries = @()
    foreach ($item in $parsedHashJson) { $hashEntries += $item }
    if ($hashEntries.Count -eq 0) { throw 'Manifesto de hashes vazio ou invalido.' }
    foreach ($entry in $hashEntries) {
        $relative = ([string]$entry.file).Replace('/', '\')
        if ([string]::IsNullOrWhiteSpace($relative) -or [string]::IsNullOrWhiteSpace([string]$entry.sha256)) { throw 'Manifesto de hashes contem entrada invalida.' }
        $candidate = Join-Path $staging $relative
        if (-not (Test-Path -LiteralPath $candidate -PathType Leaf)) { throw "Arquivo listado no manifesto esta ausente: $relative" }
        $actual = (Get-FileHash -LiteralPath $candidate -Algorithm SHA256).Hash
        if ($actual -ne [string]$entry.sha256) { throw "Falha de integridade em: $relative" }
    }
    Write-Host "BACKUP_INTEGRITY_OK=true | arquivos=$($hashEntries.Count)" -ForegroundColor Green
    ...
    foreach ($required in @('DB_HOST','DB_PORT','DB_USER','DB_PASSWORD','DB_NAME')) {
        if (-not $envMap.ContainsKey($required) -or [string]::IsNullOrWhiteSpace([string]$envMap[$required])) { throw "Configuracao obrigatoria ausente no .env do backup: $required" }
    }
    $dbName = [string]$envMap['DB_NAME']; $appUser = [string]$envMap['DB_USER']; $appPassword = [string]$envMap['DB_PASSWORD']; $dbPort = [string]$envMap['DB_PORT']; $dbPortForCleanup = $dbPort
    if ([string]$envMap['DB_HOST'] -notin @('127.0.0.1','localhost','::1')) { throw "O backup nao aponta para banco local. DB_HOST=$($envMap['DB_HOST'])" }
    ...
    if ($TestMode) {
        ...
        $testDbName = "bacbo_restore_test_$testId"; $testAppUser = "bacbo_rt_$testId"; $testTargetRoot = Join-Path $env:TEMP "Bacbo_Restore_Test_$testId"
        $dbName = $testDbName; $appUser = $testAppUser; $TargetRoot = $testTargetRoot; $Force = $true
        ...
        if ([string]::IsNullOrWhiteSpace($TargetRoot)) {
            ...
            if ([string]::IsNullOrWhiteSpace($typedTarget)) { $TargetRoot = $defaultTargetRoot } else { $TargetRoot = $typedTarget.Trim().Trim('"') }
        }
    }
    ...
    if (Test-Path -LiteralPath $TargetRoot) {
        ...
        if ($existing.Count -gt 0 -and -not $Force) { throw "O destino ja contem arquivos: $TargetRoot. Use -Force somente se deseja restaurar por cima do destino existente." }
    } else { New-Item -ItemType Directory -Path $TargetRoot -Force | Out-Null }
    ...
    if ($TestMode) {
        $targetEnvContent = Get-Content -LiteralPath $targetEnv | ForEach-Object {
            if ($_ -match '^\s*DB_NAME\s*=') { "DB_NAME=$dbName" } elseif ($_ -match '^\s*DB_USER\s*=') { "DB_USER=$appUser" } else { $_ }
        }
...
"@
    ...
    Invoke-MariaDb -Exe $mariaExe -Password $rootPassword -Arguments @('--host=127.0.0.1',"--port=$dbPort",'--user=root') -InputFile $adminSqlFile | Out-Null

    Write-Step 'Importando banco de dados'
    Invoke-MariaDb -Exe $mariaExe -Password $rootPassword -Arguments @('--host=127.0.0.1',"--port=$dbPort",'--user=root',$dbName) -InputFile $dumpFile | Out-Null
    ...
    $validation = Invoke-MariaDb -Exe $mariaExe -Password $appPassword -Arguments @('--host=127.0.0.1',"--port=$dbPort","--user=$appUser",'--batch','--skip-column-names',$dbName,'-e',"SELECT CONCAT(DATABASE(),'|',COUNT(*)) FROM information_schema.tables WHERE table_schema='$dbName' AND table_type='BASE TABLE';")
    ...
    $restoredDb = $parts[0]; $restoredTables = [int]$parts[1]; $expectedTables = [int]$manifest.database.table_count
    if ($restoredDb -ne $dbName -or $restoredTables -ne $expectedTables) { throw "Validacao final falhou. banco=$restoredDb tabelas=$restoredTables esperado=$expectedTables" }
    Write-Host "Banco=$restoredDb | tabelas=$restoredTables"

    Write-Step 'Validando runtimes da aplicacao'
    $nodeOk = [bool](Get-Command node.exe -ErrorAction SilentlyContinue); $npmOk = [bool](Get-Command npm.cmd -ErrorAction SilentlyContinue); $pythonOk = [bool](Get-Command python.exe -ErrorAction SilentlyContinue)
    Write-Host "NODE_INSTALLED=$nodeOk"; Write-Host "NPM_INSTALLED=$npmOk"; Write-Host "PYTHON_INSTALLED=$pythonOk"
    ...
    Write-Host ''
    if ($TestMode) { Write-Host 'RESTAURACAO_TESTE_OK=false' -ForegroundColor Red } else { Write-Host 'RESTAURACAO_COMPLETA_OK=false' -ForegroundColor Red }
    throw
}
finally {
    if ($TestMode -and -not [string]::IsNullOrWhiteSpace([string]$rootPassword) -and -not [string]::IsNullOrWhiteSpace([string]$testDbName)) {
        try {
            $cleanupSql = "DROP DATABASE IF EXISTS ``$testDbName``;`nDROP USER IF EXISTS '$testAppUser'@'127.0.0.1';`nDROP USER IF EXISTS '$testAppUser'@'localhost';`nFLUSH PRIVILEGES;"
            $cleanupSqlFile = Join-Path $staging 'restore-test-cleanup.sql'
            $cleanupSql | Set-Content -LiteralPath $cleanupSqlFile -Encoding UTF8
            Invoke-MariaDb -Exe $mariaExe -Password $rootPassword -Arguments @('--host=127.0.0.1',"--port=$dbPortForCleanup",'--user=root') -InputFile $cleanupSqlFile | Out-Null
            Write-Host 'RESTORE_TEST_DATABASE_CLEANUP=true' -ForegroundColor Green
        } catch { Write-Warning "Nao foi possivel limpar automaticamente o banco/usuario temporario: $($_.Exception.Message)" }
    }
    $rootPassword = $null
    if ($TestMode -and -not [string]::IsNullOrWhiteSpace([string]$testTargetRoot) -and (Test-Path -LiteralPath $testTargetRoot)) { Remove-Item -LiteralPath $testTargetRoot -Recurse -Force -ErrorAction SilentlyContinue }
